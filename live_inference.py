...
# --- Paths ---
weights_path = "/Users/scramer/Documents/mars_object_detection/runs/detect/output_clean/mars_yolo_fpga/weights/best.pt"
test_images_dir = (
    "/Users/scramer/Documents/mars_object_detection/data/yolo_mars/images/test"
)
test_labels_dir = (
    "/Users/scramer/Documents/mars_object_detection/data/yolo_mars/labels/test"
)
yaml_path = (
    "/Users/scramer/Documents/mars_object_detection/data/yolo_mars/mars.yaml"
)

# --- Class Definitions & Unified Color Palette (BGR) ---
...
# Synchronized colors for both GT and Prediction:
# 0: Soil (Orange), 1: Bedrock (Cyan), 2: Sand (Yellow), 3: Big Rock (Magenta)
CLASS_COLORS = {
...
def draw_crisp_box(img, x1, y1, x2, y2, label_text, color):
  ...
  # Calculate pill box size for text label
  ...
  cv2.rectangle(
      img, (x1, pill_y1), (x1 + tw + 10, pill_y2), color, -1, cv2.LINE_AA
  )

  # Black text for optimal contrast on bright colored pills
...
def get_ground_truth_view(img_raw, label_path, target_size=(768, 768)):
  # Upscale raw image FIRST so vector text renders ultra-crisp
...
def get_prediction_view(
    model, img_path, img_raw, target_size=(768, 768), conf_thresh=0.30
):
  # Upscale raw image FIRST
  img_disp = cv2.resize(img_raw, target_size, interpolation=cv2.INTER_CUBIC)
  h_disp, w_disp = img_disp.shape[:2]

  # Run inference on native 256x256 image
...
while True:
  ...
  # Generate high-res display panels
  gt_panel = get_ground_truth_view(raw_img, label_path, target_size=(700, 700))
  pred_panel = get_prediction_view(
      model, img_path, raw_img, target_size=(700, 700)
  )

  # --- Build UI Layout Canvas ---
  header_h = 60
  footer_h = 35
  sep_w = 6
  panel_w = 700
  ...
  canvas[:] = (20, 20, 20)  # Premium Dark Theme Background
  ...
  # Header Section - File Counter (Aligned Right, No Overlaps)
